...
export function buildDashboardSnapshot({
  ...
  const apyByAddress = new Map(
    apyResult.apys.map((entry) => [entry.address, entry.apy]),
  );
  const validators: ValidatorSnapshot[] = systemState.activeValidators.map((validator) => ({
    ...
    exchangeRatesSize: validator.exchangeRatesSize
      ? Number(validator.exchangeRatesSize)
      : undefined,
  }));
  ...
  const epochHistory = sortedEpochs
    .map((epoch) => {
      ...
      const currentValidators = new Set(
        epoch.validators.map((validator) => validator.iotaAddress),
      );
      ...
      return {
        ...
        timestampMs: Number(
          epoch.endOfEpochInfo?.epochEndTimestamp ?? epoch.epochStartTimestamp,
        ),
        totalTransactions,
  ...
  const checkpointIntervalMs =
    checkpointDeltas.length > 0
      ? checkpointDeltas.reduce((sum, delta) => sum + delta, 0) /
        checkpointDeltas.length
      : 0;

  const averageTransactionsPerCheckpoint =
    checkpointSample.length > 0
      ? checkpointSample.reduce(
          (sum, checkpoint) => sum + (checkpoint.transactions?.length ?? 0),
          0,
  ...
  const networkTps =
    checkpointIntervalMs > 0
      ? (averageTransactionsPerCheckpoint / checkpointIntervalMs) * 1000
      : 0;
  ...
  const latestCheckpointTransactionsCount =
    checkpointsDescending[0]?.transactions?.length ?? 0;

  const recentIntervalSource =
    recentWindow.length >= 2
      ? recentWindow.slice().reverse()
      : checkpointsAscending.slice(-12);
